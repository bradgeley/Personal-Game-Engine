...
void SAnimation::Startup()
{
	...
	AddReadDependencies<CRender, CMovement, CTransform, CHealth, SCCamera>();
}
...
void SAnimation::Run(SystemContext const& context)
{
    ...
    for (auto it = g_ecs->Iterate<CRender, CAnimation, CTransform>(context); it.IsValid(); ++it)
    {
        ...
        CMovement const* movement = g_ecs->GetComponent<CMovement>(it.m_currentIndex);
		CHealth const* health = g_ecs->GetComponent<CHealth>(it.m_currentIndex);
        ...
        // Update movement/idle animation (only if not dead)
